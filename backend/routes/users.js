...
router.post('/signUp', function (req, res) {
  ...
  connection.query('SELECT userid FROM users WHERE userid = "' + user.userid + '"', function (err, row) {
    if (row[0] == undefined){ //  동일한 아이디가 없을경우,
      ...
      connection.query('INSERT INTO users (userid,name,address,tel,password,email) VALUES ("' + user.userid + '","' + user.name + '","' + user.address + '","' + user.tel + '","' + encryptedPassword + '","' + user.email + '")', user, function (err, row2) {
        if (err) throw err;
      });
      res.json({
        success: true,
